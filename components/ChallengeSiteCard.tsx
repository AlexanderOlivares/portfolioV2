...
const ChallengeSiteCard: React.FC<IChallengeSiteCardProps> = ({ site }) => {
  return (
    <div className="py-3 bg-emerald text-tan shadow-lg shadow-emerald rounded-md border-tan">
      ...
      <div className="w-96 h-52 text-center">
        ...
        {!!site.solved && <div className="px-2 text-2xl">Challenges solved {site.solved}</div>}
        <div className="px-2 mb-1 text-lg">{site.shortDescrip}</div>
        {site.longDescrip && <div className="text-center text-lg">{site.longDescrip}</div>}
      </div>
      {site.site == "Codewars" && (
        <div className="text-center">
          <Image loader={imageLoader} src={site.img} alt={site.site} width="200" height="50" />
        </div>
      )}
